...
from src.data import CycleDataset
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == 'fit' or stage is None:
            self.train = CycleDataset('train', self.config, verbose=self.verbose)
            self.valid = CycleDataset('valid', self.config, verbose=self.verbose)

        if stage == 'test':
            self.test = CycleDataset('test', self.config, verbose=self.verbose)
...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(nb_downsampling):
            conv = nn.Conv2d(out_channels * f, out_channels * 2 * f,
                             kernel_size = 3,
                             stride = 2,
                             padding = 1)
        ...
        for i in range(num_resnet_layer):
            res_blk = ResBlock(in_channels = out_channels * f, apply_dp = apply_dp)
            self.layers += [res_blk]

        for i in range(nb_downsampling):
            conv = nn.ConvTranspose2d(out_channels * f, out_channels * (f//2), 3, 2,
                                      padding = 1,
                                      output_padding = 1)
...
class ConstraintLayer(nn.Module):
    ...
    def model_inv_norm_minus1_to_plus1_transform(self, x, is_input=True):
        ...
        if is_input:
            ...
            historical_reference_max = torch.tensor(3.7)

        else:
            historical_reference_min = torch.tensor(0)
            historical_reference_max = torch.tensor(3.9)
...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        for idx in range(1, nb_layers):
            conv = nn.Conv2d(out_channels * in_f, out_channels * out_f,
                             kernel_size = 4,
                             stride = 2,
                             padding = 1)
        ...
        conv = nn.Conv2d(out_channels * out_f,
                         out_channels = 1,
                         kernel_size = 4,
                         stride = 1,
                         padding = 1)
...
class CycleGAN(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.d_A = init(Discriminator(in_channels = 1, out_channels = 64, nb_layers = discriminator_layer))
        self.d_B = init(Discriminator(in_channels = 1, out_channels = 64, nb_layers = discriminator_layer))
        self.g_A2B = init(Generator(in_channels = 1, out_channels = 64,
                                    apply_dp = False, num_resnet_layer=num_resnet_layer))
        self.g_B2A = init(Generator(in_channels = 1, out_channels = 64,
                                    apply_dp = False, num_resnet_layer=num_resnet_layer))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            self.log_dict(dict_, on_step = False,
                                 on_epoch = True,
                                 prog_bar = True,
                                 logger = True)
        ...
        if optimizer_idx == 1:
            ...
            self.log("d_A_train_loss", d_A_loss,
                     on_step = True,
                     on_epoch = True,
                     prog_bar = True,
                     logger = True)
        ...
        if optimizer_idx == 2:
            ...
            self.log("d_B_train_loss", d_B_loss,
                     on_step = True,
                     on_epoch = True,
                     prog_bar = True,
                     logger = True)
